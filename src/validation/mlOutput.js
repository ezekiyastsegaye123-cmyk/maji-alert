...
const mlOutputSchema = z.object({
  ...
  operational_accuracy: z.number().min(0.0).max(1.0).optional(),
  severe_drought_detection_accuracy: z.number().min(0.0).max(1.0).optional(),
  normal_year_accuracy: z.number().min(0.0).max(1.0).optional(),
  extreme_deficit_accuracy: z.number().min(0.0).max(1.0).optional(),
  combined_drought_risk: z.number().min(0.0).max(1.0).optional(),
  drought_risk_tier: z.string().optional(),
